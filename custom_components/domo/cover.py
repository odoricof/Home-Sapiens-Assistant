...
from .platforms.openings import DomoOpening

_LOGGER = logging.getLogger(__name__)


async def async_setup_entry(hass, entry, async_add_entities):
    """Setup cover platform per le aperture motorizzate."""
    from .platforms.openings import get_all_openings
    
    openings = get_all_openings()
    if not openings:
        _LOGGER.debug("No openings found yet")
        return
    
    # Crea un dispositivo virtuale che contiene tutte le aperture motorizzate
...
class DomoCoverEntity(CoverEntity):
    """Cover entity per aperture motorizzate Domo."""
